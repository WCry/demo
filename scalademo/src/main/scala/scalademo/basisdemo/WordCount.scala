...
import scalademo.util.AppStartInfo
...
object WordCount {
  def main(args: Array[String]): Unit = {
    ...
    conf.setAppName("FirstSparkApplication")
     ...
     */
    val appPath = AppStartInfo.AppStartPath;
    //文件的路径，最小并行度（根据机器数量来决定）
    //val lines:RDD[String]= sc.textFile("F://spark//spark-1.6.2-bin-hadoop2.6//README.md", 1)
    //读取本地文件，并设置Partition = 1
    //读取本地文件，并设置Partition = 1   //类型推导得出lines为RDD
    val lines = sc.textFile(appPath+"/config/DegreedRelationship.txt", 1)
    /**
